...
<body class="bg-light">
  <div class="d-flex">
        ...
        <aside class="bg-white border-end p-3" style="width: 270px; height: 100%;">
          <div class="mb-4 position-relative">
            <form action="search_user.php" method="GET" class="position-relative mb-4">
              <input type="text" name="q" class="form-control ps-5 rounded-pill" placeholder="Search users..."
                required />
          ...
          <nav class="nav flex-column">
            <a href="#" class="nav-link text-dark d-flex align-items-center mb-2"><i
                class="fa fa-home me-2 text-primary"></i><span>Home</span></a>
            <a href="profile.php" class="nav-link text-dark d-flex align-items-center mb-2"><i
                class="fa fa-user me-2 text-primary"></i><span>Profile</span></a>
            <a href="notification.php" class="nav-link text-dark d-flex align-items-center mb-2"><i
                class="fa fa-bell me-2 text-primary"></i><span>Notifications</span></a>
            <a href="messages.php" class="nav-link text-dark d-flex align-items-center mb-2"><i
                class="fa fa-envelope me-2 text-primary"></i><span>Messages</span></a>
            <a href="?logout=1" class="nav-link text-danger d-flex align-items-center mt-4"><i
                class="fa fa-sign-out-alt me-2"></i><span>Logout</span></a>
          </nav>
        </aside>
      </div>
    </div>
        ...
        <div class="bg-white p-3 rounded shadow-sm mb-3">
          <div class="d-flex justify-content-between align-items-center mb-2">
            <div class="d-flex align-items-center">
              <img
                src="<?= !empty($post['profile_pic']) ? htmlspecialchars($post['profile_pic']) : 'gambar/profile_pic.jpg' ?>"
          ...
          <!-- Tombol Komentar -->
          <div class="me-3">
            <a href="#" class="comment-btn" data-bs-toggle="modal" data-bs-target="#commentModal"
              data-postid="<?= $post['id_post'] ?>">
    ...
    <aside class="bg-white border-start p-3 d-none d-xl-block sidebar-right" style="width: 300px">
      <div class="d-flex align-items-center mb-3 border-bottom pb-3">
        <img
          src="<?= !empty($user_data['profile_pic']) ? htmlspecialchars($user_data['profile_pic']) : 'gambar/profile_pic.jpg' ?>"
          alt="User Avatar" class="rounded-circle me-2" width="40" height="40" />
        <div>
  ...
  <button class="btn btn-primary rounded-circle position-fixed"
    style="width: 45px; height: 45px; bottom: 20px; right: 20px;" data-bs-toggle="modal" data-bs-target="#postModal">
      ...
      <div class="modal-content">
        <form action="create_post.php" method="POST" enctype="multipart/form-data">
          ...
          <div class="modal-body">
            <textarea name="content" class="form-control mb-3" rows="3" placeholder="What's on your mind?"
              required></textarea>
        ...
        <div class="modal-body">
          ...
          <form id="commentForm">
            <div class="input-group input-group-sm">
              <input type="text" name="comment" id="commentInput" class="form-control" placeholder="Write a comment..."
                required />
...
<script>
  ...
  document.querySelectorAll('.comment-btn').forEach(btn => {
    btn.addEventListener('click', function () {
      ...
      document.getElementById('commentsContainer').innerHTML = 'Loading...';

      fetch('get_comments.php?post_id=' + currentPostId)
        .then(res => res.json())
        .then(data => {
          if (data.status === 'success') {
            ...
            data.comments.forEach(comment => {
              ...
              div.innerHTML = `
                <img src="${comment.profile_pic || 'gambar/profile_pic.jpg'}" class="rounded-circle me-2" width="32" height="32">
                <div>
                  <div class="fw-semibold small mb-1">${comment.username} <span class="text-muted small">${comment.created_at}</span></div>
                  <div class="text-muted small">${comment.comment.replace(/\n/g, "<br>")}</div>
                </div>
              `;
  ...
  document.getElementById('commentForm').addEventListener('submit', function (e) {
    ...
    fetch('add_comment.php', {
      ...
      body: 'post_id=' + currentPostId + '&comment=' + encodeURIComponent(commentText)
    })
      .then(res => res.json())
      .then(data => {
        if (data.status === 'success') {
          ...
          // Reload komentar
          document.querySelector(`.comment-btn[data-postid="${currentPostId}"]`).click();
        } else {
